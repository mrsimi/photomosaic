...
class Photomosaic:
    ...
    def process(self):
        ...
        mosaic_template = face_im_arr[::(main_image_height//self.target_res[0]),::(main_image_height//self.target_res[1])]
        
        print(f"Main Image process completed: {main_image_height} by {main_image_width}")
        
        tile_images = self.load_tile_images(self.tile_images_path)
        ...
        canvas.save(f'{self.output_dir}/{self.generate_random_string(5)}_mosaic.jpg');
